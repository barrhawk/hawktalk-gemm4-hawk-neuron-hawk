...
class NeuronAttentionBase(nn.Module):
    ...
    def perform_prefix_prefill(self, Q, K, V, q_len, bsz, attention_mask, past_key_value, active_mask) -> Tensor:
        ...
        if flash_attn_strategy != FlashAttentionStrategy.NONE:
            ...
            # PATCH_A: native prefix path ignores attention_mask; apply it to the prior
            # so prefixes that are not exactly a prefix bucket do not attend to padded blocks.
            if attention_mask is not None and attention_mask.dim() == 2 and attention_mask.shape[-1] == prior_scores.shape[-1]:
                _prior_mask = attention_mask.to(torch.bool)[:, None, None, :]
                prior_scores = torch.where(_prior_mask, prior_scores, torch.finfo(prior_scores.dtype).min)
    ...
    def perform_prefix_prefill_windowed_attn(self, Q, K, V, q_len, bsz, attention_mask, window_size, past_key_value, active_mask) -> Tensor:
        """SWA_PATCH_P3: chunked / prefix prefill for a sliding-window layer (native path only).

        Contract (block-KV prefix-caching CTE, same as perform_prefix_prefill):
          past_key_value : (K_prior, V_prior) BHSD, row r == absolute position r
          attention_mask : (bsz, prior_len) 2-D validity mask, True for r < P (P = prior used len)
          active_mask    : (bsz, 1, q_len, q_len) causal mask for this chunk
        Query i is at absolute position P+i. With window W (W keys incl. self):
          prior key at absolute a visible iff a < P and (P+i) - a < W
          active key m visible iff m <= i and i - m < W
        Only prior rows [P-W, P) can ever be visible, so gather exactly those W rows per
        batch (tail of the block table) instead of scoring against the whole prior_len cache.
        Prior row j (absolute P-W+j) is visible to query i iff j > i and P-W+j >= 0.
        """
        assert window_size and window_size > 0, "perform_prefix_prefill_windowed_attn needs window_size"
        assert not self.k_cache_transposed, "k_cache_transposed unsupported on the SWA prefix-prefill path"
        assert attention_mask is not None and attention_mask.dim() == 2, \
            f"SWA prefix prefill expects the 2-D (bsz, prior_len) prior mask, got {None if attention_mask is None else tuple(attention_mask.shape)}"
        W = int(window_size)
        K_prior_full, V_prior_full = past_key_value[0], past_key_value[1]   # (bsz, Hkv, prior_len, D)
        prior_len = K_prior_full.shape[2]
        device = Q.device

        # P per batch == number of valid prior rows == absolute position of query 0
        P = torch.sum(attention_mask.to(torch.int64), dim=1)                  # (bsz,)
        i = torch.arange(q_len, device=device, dtype=torch.int64)

        if os.environ.get("SWA_FULL_PRIOR", "0") == "1" or prior_len <= W:
            # Fallback / small prior: score against the whole prior with the absolute-position window rule.
            a = torch.arange(prior_len, device=device, dtype=torch.int64)      # absolute prior positions
            q_abs = P[:, None] + i[None, :]                                     # (bsz, q_len)
            prior_mask = (a[None, None, :] < P[:, None, None]) & ((q_abs[:, :, None] - a[None, None, :]) < W)
            prior_mask = prior_mask[:, None, :, :]                              # (bsz, 1, q_len, prior_len)
            K_prior, V_prior = K_prior_full, V_prior_full
        else:
            j = torch.arange(W, device=device, dtype=torch.int64)               # (W,)
            abs_prior_pos = P[:, None] - W + j[None, :]                         # (bsz, W)
            gather_pos = abs_prior_pos.clamp(min=0, max=prior_len - 1)
            gidx = gather_pos[:, None, :, None].expand(bsz, K_prior_full.shape[1], W, self.head_dim)
            K_prior = torch.gather(K_prior_full, dim=2, index=gidx)             # (bsz, Hkv, W, D)
            V_prior = torch.gather(V_prior_full, dim=2, index=gidx)
            prior_mask = (j[None, None, :] > i[None, :, None]) & (abs_prior_pos[:, None, :] >= 0)
            prior_mask = prior_mask[:, None, :, :]                              # (bsz, 1, q_len, W)

        # sliding rule on the active block too (matters when q_len > W)
        sw_active = (i[:, None] - i[None, :]) < W                              # (q_len, q_len)
        active_mask = active_mask.to(torch.bool) & sw_active[None, None, :, :]

        # native path only (the gemma4 port forces attn_kernel_enabled=False)
        K_active = repeat_kv(K, self.num_key_value_groups)
        V_active = repeat_kv(V, self.num_key_value_groups)
        K_prior = repeat_kv(K_prior, self.num_key_value_groups)
        V_prior = repeat_kv(V_prior, self.num_key_value_groups)
        ...
        softmax_prior, softmax_active = manual_softmax(prior_scores, active_scores, True)
        softmax_prior, softmax_active = softmax_prior.to(Q.dtype), softmax_active.to(Q.dtype)
        attn_output = torch.matmul(softmax_prior, V_prior) + torch.matmul(softmax_active, V_active)
        return attn_output, FlashAttentionStrategy.NONE
    ...
    def windowed_attention_forward(
        ...
        **kwargs,
        ...
        if self.neuron_config.is_prefix_caching:
            # SWA_PATCH_P1: parity with standard_causal_attention_forward.
            # Under prefix caching / emulated chunked prefill a sliding layer
            # also receives past_key_value (the cached prior) during context
            # encoding. Smallest prefix bucket is 128, so q_len >= 128 means
            # a C-token chunk -> perform_prefix_prefill_windowed_attn, not
            # compute_for_token_gen. Must precede the block-TKG NKI branch.
            is_token_gen = is_token_gen and q_len < 128
        ...
        if is_token_gen:
            ...
            if not self.neuron_config.is_block_kv_layout:
                # Ring-buffer cache: keep only the last window, rolled into ring order.
            ...
            # SWA_PATCH_P2: block-KV writer addresses rows by the absolute slot_mapping and
            # takes n_active_tokens from K.shape[2]; the full q_len-row chunk passes through.
            # The window trim happens on the READ side (perform_prefix_prefill_windowed_attn).
