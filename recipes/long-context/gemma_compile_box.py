#!/usr/bin/env python
"""Compile SWA-patched gemma4-E2B with block-KV + prefix caching (emulated chunked prefill), TP2 bf16.
CTE buckets [512,2048] x prefix buckets [512,1024,1536] (+ no-prefix) + TKG = 9 NEFFs."""
...
OUT = os.environ.get("GOUT", os.path.expanduser("~/gemma_pc"))
...
NBLK = int(os.environ.get("PA_NUM_BLOCKS", "256"))

print("SWA_FULL_PRIOR env =", os.environ.get("SWA_FULL_PRIOR", "<unset>"), flush=True)
t0 = time.time()
...
print("CFG buckets", cfg.neuron_config.buckets, "layer_types sliding=%d full=%d" % (
    ...
    "sliding_window", cfg.sliding_window, "pa_num_blocks", nc.pa_num_blocks, flush=True)
...
neffs = glob.glob(os.path.join(OUT, "**/*.neff"), recursive=True)
print("NEFF count", len(neffs))
for n in sorted(neffs):
    print("  ", os.path.relpath(n, OUT), os.path.getsize(n))
